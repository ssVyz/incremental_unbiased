...
func MakeReverseComplement(oligo string) (string, error) {
	...
	for i := 0; i < oligoLength; i++ {
		...
		result = result + complements[strings.ToUpper(string(currentPos))]
		idx--
	}
	return result, nil
} 
...
// collapses a slice of canonical bases into a iupac code
func collapseIupac(bases []byte) (byte, error) {
	...
	for key, slce := range identical {
		// WARNING: This has to be updated if the list of characters in identical changes
...
// Writes a fasta file at path, containing the provided slice of Seqr
...
func MakeConsensus(sl []Seqr) (string, error) {
	...
	for i := 0; i < leng; i++ {
		posBuild := posBase{}
		for j := 0; j < len(sl); j++ {
			if isCanonBase(sl[j].Seq[i]) == false {
				return "", fmt.Errorf("Consensus builder detected a non-canonical base in the sequence set.")
			}
		}
		for j := 0; j < len(sl); j++ {
			if sl[j].Seq[i] == 'A' {
